...
settings = directory['settings']['basal-ganglia']
...
M = np.loadtxt(settings['connections'],delimiter=',')
...
area = {int(k):v for k,v in settings['area'].iteritems()}
...
artist.raster_plot(v,filename='pain',duration=[params['duration']['interpattern' if 'interpattern' in condition else 'pattern'] for condition in pattern_])
#	parasite_labels=[condition if 'interpattern' not in condition else '' for condition in pattern_])


artist.firing_rate_plot(v,filename='pain',duration=[params['duration']['interpattern' if 'interpattern' in condition else 'pattern'] for condition in pattern_],
	parasite_labels=[condition if 'interpattern' not in condition else '' for condition in pattern_])